...
public enum XMaterial
{
    ...
    ORANGE_TULIP(5, new String[] { "RED_ROSE", "RED_TULIP" }), 
    ...
    ROSE_RED(1, new String[] { "INK_SACK", "RED_DYE" }), 
    ...
    OAK_WALL_SIGN(0, new String[] { "WALL_SIGN", "SIGN_POST" }), 
    ACACIA_WALL_SIGN(0, new String[] { "WALL_SIGN", "SIGN_POST" }), 
    SPRUCE_WALL_SIGN(0, new String[] { "WALL_SIGN", "SIGN_POST" }), 
    BIRCH_WALL_SIGN(0, new String[] { "WALL_SIGN", "SIGN_POST" }), 
    DARK_OAK_WALL_SIGN(0, new String[] { "WALL_SIGN", "SIGN_POST" }), 
    JUNGLE_WALL_SIGN(0, new String[] { "WALL_SIGN", "SIGN_POST" }), 
    ...
    private XMaterial(final int data, final String[] m) {
        this.m = m;
        this.data = data;
    }
    
    public ItemStack parseItem() {
        final Material mat = this.parseMaterial();
        if (isNewVersion()) {
    ...
    public static boolean isNewVersion() {
        final Material mat = Material.getMaterial("RED_WOOL");
        return mat != null;
    }
    
    public static XMaterial requestXMaterial(final String name, final byte data) {
        if (XMaterial.cachedSearch.containsKey(name.toUpperCase() + "," + data)) {
            return XMaterial.cachedSearch.get(name.toUpperCase() + "," + data);
        }
        for (final XMaterial mat : values()) {
            for (final String test : mat.m) {
                if (name.toUpperCase().equals(test) && (byte)mat.data == data) {
                    XMaterial.cachedSearch.put(test + "," + data, mat);
                    return mat;
                }
            }
        }
    ...
    public boolean isSameMaterial(final ItemStack comp) {
        ...
        final XMaterial xmat = this.fromMaterial(comp.getType());
        return this.isDamageable(xmat) && this.parseMaterial() == comp.getType();
    }
    
    public XMaterial fromMaterial(final Material mat) {
        try {
            return valueOf(mat.toString());
        }
        catch (IllegalArgumentException e) {
            for (final XMaterial xmat : values()) {
                for (final String test : xmat.m) {
    ...
    public static XMaterial fromString(final String key) {
        XMaterial xmat = null;
        try {
            xmat = valueOf(key);
            return xmat;
    ...
    public boolean isDamageable(final XMaterial type) {
        if (type == null) {
            return false;
        }
        final String[] split = type.toString().split("_");
    ...
    public Material parseMaterial() {
        final Material mat = Material.matchMaterial(this.toString());
        if (mat != null) {
            return mat;
        }
    ...
    public Material parseMaterial(final boolean item) {
...
//        }
    ...
    static {
        XMaterial.cachedSearch = new HashMap<String, XMaterial>();
    }
}
